...
def extract_profile_footprint_observations(profile: dict[str, Any]) -> list[dict[str, Any]]:
    """Extract verified external footprint observations from a company profile.
    
    Includes:
    1. Verified social profiles (LinkedIn, Facebook, Instagram, etc.) with identity score >= 0.90
    2. Company-owned site activity and metrics if publishable
    3. Official registry references and subunit locations
    """
    import hashlib
    observations: list[dict[str, Any]] = []
    ...
    # 2. Company site activity metrics if publishable
    site_digest = val.get("content_sha256") or website.get("content_sha256")
    source_url = val.get("final_url") or website.get("source_url")
    if is_publishable and site_digest and len(str(site_digest)) == 64 and source_url:
