...
def build_flexible_company_search_queries(profile: dict[str, Any]) -> list[str]:
    ...
    queries = [
        f'"{name}" {org}',
        f"{clean_name} {municipality}" if municipality else f"{clean_name} Norway",
        f'"{name}" {org} {municipality}'.strip(),
    ]
    ...
    if email_domain and "." in email_domain and email_domain not in {"gmail.com", "online.no", "hotmail.com", "outlook.com", "yahoo.com"}:
        queries.append(f"{clean_name} {email_domain}")

    if municipality:
        queries.append(f"{clean_name} {municipality}")
    queries.append(f"{clean_name} Norway")
...
def score_search_candidate(profile: dict[str, Any], result: dict[str, Any]) -> dict[str, Any]:
    ...
    publishable_candidate = (
        score >= 0.75
        and (name_in_host or email_domain_match)
        and (org_match or all_name_tokens_in_title)
    )
    return {
