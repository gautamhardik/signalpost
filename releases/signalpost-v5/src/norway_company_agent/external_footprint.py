...
def extract_profile_footprint_observations(profile: dict[str, Any]) -> list[dict[str, Any]]:
    """Extract verified external footprint observations from a company profile using modular adapters.
    
    Includes:
    1. Verified social profiles (LinkedIn, Facebook, Instagram, YouTube, X) with identity score >= 0.90
    2. Company-owned site activity and metrics if publishable
    3. Active hiring and career signals from verified company websites
    4. Verified public news and announcements
    5. Official BRREG subunit physical presences
    """
    from .adapters import extract_all_adapter_observations
    return extract_all_adapter_observations(profile)
